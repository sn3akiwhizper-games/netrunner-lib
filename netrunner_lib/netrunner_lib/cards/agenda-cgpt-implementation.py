'''
card classes of type agenda using chatGPT generated implementations
'''
...
class agenda_accelerated_beta_test_01055(Agenda):
    ...
    def play(self, player, kwargs) -> str:
        ...
        top_three_cards = player.deck[-3:]#neg b/c we use pop which grabs from end of list
        if "ice" in [card.type for card in top_three_cards]:
            selection_idx = player.prompt(f"Top three cards are: {[f'{idx}) {str(card)}' for idx, card in top_three_cards]}\nType index of card to install (must be ICE)")
...
class agenda_nisei_mk_ii_01068(Agenda):
    ...
    CGPT Notes:
        The agenda_nisei_mk_ii_01068 class extends the Agenda class and overrides its score method. When the score method is called, it increments the agenda_counters attribute and creates a new EndRunEffect object, which it attaches to the card's state attribute.

        The EndRunEffect class is a simple effect that ends the run and decrements the agenda_counters attribute of the card when triggered.

        Note that the EndRunEffect class is a stub and its implementation will depend on the rest of the game implementation. The game.end_run() method and the game.trash_card() method will need to be defined elsewhere in the game implementation.
    '''
    def __init__(self):
        ...
        self.agenda_counters = 0

    def score(self, player, game) -> str:
        '''
        Increment the agenda counter and attach the "End the run" effect.
        '''
        print(f'scoring card: {self.title}')
        self.agenda_counters += 1
        effect = EndRunEffect(self)
        self.state["effects"].append(effect)
        return "nop"

class EndRunEffect:
    '''
    An effect that ends the run when triggered.
    '''
    def __init__(self, source_card):
        self.source_card = source_card

    def trigger(self, game, trigger_input=None):
        game.end_run()
        self.source_card.agenda_counters -= 1
        if self.source_card.agenda_counters == 0:
            game.trash_card(self.source_card)
...
class agenda_astroscript_pilot_program_01081(Agenda):
    ...
    CGPT Notes:
        In the __init__ method, we set the initial values for hosted_agenda_counters and limit. In the play method, we implement the behavior for playing this card as an agenda. If the card is being installed, we look for the server where it will be installed and call the install_ice method of that server. After the card is installed, we move it to the installed_ice list of the server and discard it from the player's hand. If the card is being played in another way, we return an appropriate message.

        In the score method, we implement the behavior for scoring this card. If the card has already been scored, we return False. Otherwise, we set the is_scored attribute to True, increment the agenda_counters attribute by 1, reset the hosted_agenda_counters attribute to 0, and append the card to the player's scored_agendas list. Finally, we return a message indicating whether the player scored or stole the agenda.
    '''
    def __init__(self):
    ...
    def play(self, player, **kwargs):
        self.host = player
        player.scored_agendas.append(self)

    def score(self, player, game):
        super().score(player, game)
        self.agenda_counters += 1

    def can_advance(self, card):
        return card.type_code in ["asset", "upgrade", "agenda"] and "advancement" in card.keywords

    def can_host_agenda_counters(self):
        return self.agenda_counters < 1

    def advance_card(self, player, card):
        if not self.can_host_agenda_counters():
            return "Cannot host more than 1 agenda counter."
        if not self.can_advance(card):
            return "Card cannot be advanced."
        card.advancement_counters += 1
        self.agenda_counters += 1
        return "Card advanced successfully."
...
class agenda_breaking_news_01082(Agenda):
    ...
    CGPT Notes:
        When the card is played, the play function first calls the play function of the parent class using super().play(player, kwargs). This ensures that any standard functionality of the Agenda class is executed.

        Next, the function gives the runner 2 tags and adds the lose_tags function to the turn_actions list of the player. The lose_tags function ensures that at the end of the turn on which the agenda was scored, the runner loses 2 tags.

        Note that the TODO placeholder in the if super_result != "nop" block will need to be replaced with the relevant event code if additional functionality is added to the play function.
    '''
    def __init__(self):
    ...
    def play(self, player, kwargs) -> str:
        ...
        RETURN:
            - str: event code of what has happened/should happen
                ...
                - 'nop': no operation performed
        '''
        print(f'playing card: {self.title}')
        super_result = super().play(player, kwargs)
        if super_result != "nop":
            return super_result
        else:
            player.runner.tags += 2
            player.turn_actions.append(self.lose_tags)
            return "TODO"

    def lose_tags(self, game, player):
        player.runner.tags = max(0, player.runner.tags - 2)
