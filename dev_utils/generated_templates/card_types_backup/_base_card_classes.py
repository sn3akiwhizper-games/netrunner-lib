...
class Agenda(Card):
    ...
    def play(self, player, kwargs) -> str:
        ...
        if super_result != "nop":
            ...
            return player.install_new_remote_server(self)
...
class Ice(Card):
    ...
    def play(self, player, kwargs) -> str:
        ...
        if server_id==0:
            player.hq_server.install_ice(self)
        elif server_id==1:
            player.rnd_server.install_ice(self)
        elif server_id==2:
            player.archive_server.install_ice(self)
        else:
            player.remote_servers[server_id].install_ice(self)
...
class Icebreaker(Card):
    '''
    # This implementation includes attributes for the card's name, faction, cost, text, strength, and memory. The play method adds the icebreaker to the player's list of installed icebreakers when it is played, and the trash method removes it from the player's list when it is trashed or discarded.

    # The interact_with_ice method takes an instance of the ICE class as an input and returns a Boolean value indicating whether the icebreaker can interact with the given ICE card. This method compares the strength attribute of the icebreaker with the strength attribute of the ICE card to determine if the icebreaker can interact with it.
    '''
    def __init__(self, card_json):
        super().__init__(card_json)
    ...
    def interact_with_ice(self, ice):
        if ice.strength <= self.strength:
            return True
        else:
            return False
