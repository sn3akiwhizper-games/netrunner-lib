...
class program_ninja_01027(Program):
    '''
    Interface -> 1 credit: Break 1 sentry subroutine. 3 credits: +5 strength.
    '''
    def __init__(self):
    ...
    def play(self, player, kwargs):
        ...
        if super_result != "nop":
            return super_result
        if 'credits_spent' in kwargs.keys():
            self.state["credits_spent"] = kwargs['credits_spent']
        else:
            self.state["credits_spent"] = int(generic_player_prompt(player,"Spend [1] credit for 'Break 1 sentry subroutine option --or-- Spend [3] credits for +5 strength [type 1 or 3]"))

        if self.state['credits_spent'] == 1 and player.credit_pool >= 1:
            player.credit_pool -= 1
            return self.install(player)
        elif self.state['credits_spent'] == 3 and player.credit_pool >= 3:
            player.credit_pool -= 3
            return self.install(player)
        else:
            print(f"invalid input to program_ninja_01027:\nINPUT:{self.state['credits_spent']}\nCREDITS:{player.credit_pool}")
            return "error"
